...
class Cryozonic_Stripe_Model_Standard extends Mage_Payment_Model_Method_Abstract {
    ...
    const MODULE_VERSION = "2.5.7";
    const MODULE_URL = "http://store.cryozonic.com/magento-extensions/stripe-payments.html";
    ...
    protected function getAvsFields($card)
    {
        ...
        if (empty($billingInfo))
            throw new \Stripe\Error("You must first enter your billing address.");
        else
        {
    ...
    private function listCards($customerStripeId, $params = array())
    {
        try
        {
            $cards = \Stripe\Customer::retrieve($customerStripeId)->sources;
            if (!empty($cards))
            {
                $params['object'] = 'card';
    ...
    {
        $method = $this->getStore()->getConfig('payment/cryozonic_stripe/stripe_js');
        if (empty($method))
            return 0;
        else
            return $method;
    ...
    public function initiate3DSecureAuthentication($cardToken, $fingerprint)
    {
        ...
        return \Stripe\ThreeDSecure::create($params);
    }
