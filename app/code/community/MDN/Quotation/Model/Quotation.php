...
class MDN_Quotation_Model_Quotation extends Mage_Core_Model_Abstract {
     ...
     */
    public function NotifyCustomer($needAttachment) {
        return Mage::getModel('Quotation/Quotation_Notification')->NotifyCustomer($this, $needAttachment);
    }
    ...
    public function addHistory($message) {
        ...
        $model->setqh_quotation_id($this->getId())
                ...
                ->setqh_date(date('Y-m-d H:i:s', Mage::getModel('core/date')->timestamp()))
                ->setqh_user($username)
                ->save();
    }
    ...
    public function CalculateQuotationPriceHt() {
        ...
        //discount in value
        $price = $price - $this->getdiscount_value();
    ...
    public function getDiscountValueWithTax()
    {
        $productTaxId = Mage::getStoreConfig('quotation/general/quotation_tax_class');
        $ShippingAddress = $this->getCustomer()->getPrimaryShippingAddress();
        $BillingAddress = $this->getCustomer()->getPrimaryBillingAddress();
        ...
        return $this->GetTaxForProductFake($this->getdiscount_value(), $productTaxId, $ShippingAddress, $BillingAddress, $CustomerTaxClass, $store);
    }
    ...
    public function GetProductPrice($product, $price_product_ht, $qty = 1) {
        ...
        $customerWebsiteId = $this->getWebsiteId();
    ...
    public function getShippingTaxRate() {
        $helper = Mage::helper('tax');
        $storeId = $this->getStoreId();
        $shippingClassId = $helper->getShippingTaxClass($storeId);
