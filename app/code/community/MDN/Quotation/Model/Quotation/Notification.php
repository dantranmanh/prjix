...
class MDN_Quotation_Model_Quotation_Notification extends Mage_Core_Model_Abstract {
     ...
     */
    public function NotifyCustomer($quote, $needAttachment) {
        $translate = Mage::getSingleton('core/translate');
        $translate->setTranslateInline(false);

        $templateId = Mage::getStoreConfig('quotation/quote_notification/email_template', $quote->getStoreId());
        $identityId = Mage::getStoreConfig('quotation/quote_notification/email_identity', $quote->getStoreId());
    ...
    public function NotifyCreationToAdmin($quote) {
        ...
        $templateId = Mage::getStoreConfig('quotation/quotation_request/email_template', $quote->getStoreId());
        $identityId = Mage::getStoreConfig('quotation/quotation_request/email_identity', $quote->getStoreId());
        $sendTo = Mage::getStoreConfig('quotation/quotation_request/send_to', $quote->getStoreId());

        $url = Mage::helper('adminhtml')->getUrl('adminhtml/Quotation_Admin/edit', array('quote_id' => $quote->getId()));
        $data = array
            (
