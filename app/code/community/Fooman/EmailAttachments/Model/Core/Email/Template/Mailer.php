...
class Fooman_EmailAttachments_Model_Core_Email_Template_Mailer extends Mage_Core_Model_Email_Template_Mailer
{
    ...
    public function send()
    {
        ...
        while (!empty($this->_emailInfos)) {
            ...
            //Attaching shippingrate detail into order email - HungDQ
            $templateId = $this->getTemplateId();
            if ($templateId == Hxtech_Logistic_Model_Logistic::LOGISTIC_ORDER_EMAIL_TEMPLATE || $templateId == Hxtech_Logistic_Model_Logistic::CUSTOMER_ORDER_EMAIL_TEMPLATE || $templateId == Hxtech_Logistic_Model_Logistic::DOCUMENT_ORDER_EMAIL_TEMPLATE) {
                $mail = $emailTemplate->getMail();
                $model = Mage::getModel('logistic/csv_order');
                $content = $model->getCsv();
                $attachment = $mail->createAttachment($content);
         
                $attachment->type        = 'text/csv';
                $attachment->disposition = Zend_Mime::DISPOSITION_INLINE;
                $attachment->filename = 'order-details.csv';
    ...
    public function dispatchAttachEvent($emailTemplate, $emailInfo)
    {
        ...
        switch ($this->getTemplateId()) {
            ...
            case Hxtech_Logistic_Model_Logistic::CUSTOMER_ORDER_EMAIL_TEMPLATE:
            case Hxtech_Logistic_Model_Logistic::LOGISTIC_ORDER_EMAIL_TEMPLATE:
            case Hxtech_Logistic_Model_Logistic::DOCUMENT_ORDER_EMAIL_TEMPLATE:
