...
class Fooman_PdfCustomiser_Model_Mypdf extends TCPDF
{
    ...
    public function prepareLineItems(Fooman_PdfCustomiser_Helper_Pdf $helper, $printItem, $order = null)
    {
        ...
        foreach ($printItem->getAllItems() as $item) {
            ...
            if ($item instanceof Mage_Sales_Model_Order_Item) {
                ...
                if($pdfTemp['qty_ordered'] == 0){
                    $pdfTemp['qty_ordered'] = $item->getQty();
                }
    ...
    public function getItemNameAndSku($item, $helper)
    {
        ...
        if ($attributeCodes) {
            $barcodeAttributeCodes = array('product_barcode_image', 'product_barcode_inner_image', 'product_barcode_outer_image');
                            ...
                            if ($attribute->getBackendModel() == 'weee/attribute_backend_weee_tax') {
                                ...
                                if (is_array($attributeValue) && !empty($attributeValue)) {
                                    ...
                                    if (!$return[$customName]) {
                                        // #211 - Adding barcode image to pdf template - HUNGDQ. 
                                        if(in_array($attributeCode, $barcodeAttributeCodes)){
                                            $customValue = $helper->fixEncoding($product->getDataUsingMethod($attributeCode));
                                            if($customValue != "no_selection" && $customValue != ""){
                                                $barcodeUrl = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'catalog/product' . $customValue;
                                                $return[$customName] = '<img src="'.$barcodeUrl.'" width="250px" height="125px" />';
                                            }else{
                                                $return[$customName] = "";
                                            }
