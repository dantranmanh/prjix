...
//                $weight = Mage::getStoreConfig('amxsearch/weights/weight_' . $i);
    ...
    {
        if (sizeof($preparedTerms) > self::MAX_TERMS) { // Limit terms count to avoid "Thread stack overrun" error
            $preparedTerms = array_slice($preparedTerms, 0, self::MAX_TERMS);
        }

        $cond = 0;

        $queryText = $this->_getWriteAdapter()->quote($queryText);

        $configAttributes = $this->_getConfigAttributes();

        $pies = array();
        $repl = '(MATCH (`:col_name`) AGAINST (:query in boolean mode) * :weight)';
//        $repl .= ' + (MATCH (`:col_name`) AGAINST (:query_text) * 5 * :weight)';
        $repl .= ' + if (LOCATE(LCASE(:query_text), LCASE(`:col_name`)) != 0, 5 * :weight, 0)';
        ...
        foreach ($configAttributes as $code => $attributeData) {
            ...
            $pies[] = "(MATCH (`" . $this->_getAttributeCol($attributeData) . "`) AGAINST ($queryText in boolean mode) * $weight * 2)";
        }
        
        $pies[] = "(MATCH (s.data_index) AGAINST ($queryText in boolean mode) * 4)";
    ...
    public function prepareResult($object, $queryText, $query)
    {
        ...
        if (!empty($queryText) && $query->getStoreId()) {
    ...
    protected function useFoundData()
    {
        return isset($this->_foundData);
    }
