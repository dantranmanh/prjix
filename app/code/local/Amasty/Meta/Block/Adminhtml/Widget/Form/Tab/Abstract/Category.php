		...
		$fieldSet->addField(
			...
			array(
				...
				'name'  => $this->_prefix . 'cat_meta_title',
                                'note'  => $hlp->__('Available variables:  <br/>Parent Category - {meta_parent_category}<br/>Category Name - {name}<br/>Store View - {store_view}<br/>Store - {store}<br/>Website - {website}'),
			)
		);
