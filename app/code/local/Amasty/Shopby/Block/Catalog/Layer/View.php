...
class Amasty_Shopby_Block_Catalog_Layer_View extends Amasty_Shopby_Block_Catalog_Layer_View_Pure
{
    ...
    public function getFilters()
    {
        ...
        if ($f instanceof Amasty_Shopby_Block_Catalog_Layer_Filter_Stock
            && !$this->_notInBlock(Mage::getStoreConfig('amshopby/block/stock_filter_pos'))
        ) {
            $filters[] = $f;
        }
        ...
        if ($f && !Mage::helper('amshopby')->useSolr()
            && !$this->_notInBlock(Mage::getStoreConfig('amshopby/block/rating_filter_pos'))
        ) {
            $filters[] = $f;
        }

        $f = $this->getChild('on_sale_filter');
        if ($f instanceof Amasty_Shopby_Block_Catalog_Layer_Filter_OnSale
            && !$this->_notInBlock(Mage::getStoreConfig('amshopby/block/on_sale_filter_pos'))) {
            $filters[] = $f;
        }
        ...
        foreach ($filters as $f){
            ...
            if ($f instanceof Amasty_Shopby_Block_Catalog_Layer_Filter_OnSale) {
                $f->setPosition($filtersPositions['ama_on_sale_filter']);
            }
        }
    ...
    protected function _afterToHtml($html)
    {
        ...
        if ($pos !== false) {
            ...
            if(Mage::helper('amshopby')->getIsApplyButtonEnabled() && !$this->_notInBlock(Mage::getStoreConfig('amshopby/block/submit_position'))) {
    ...
    protected function _prepareLayout()
    {
        ...
        if (Mage::getStoreConfig('amshopby/general/on_sale_filter_pos') >= 0) {
