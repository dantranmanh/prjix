...
class Amasty_Shopby_Helper_Attributes extends Amasty_Shopby_Helper_Cached
{
	...
	public function getPositionsAttributes()
	{
        ...
        $customPositions = array(
            ...
            'ama_on_sale_filter' => Mage::getStoreConfig('amshopby/general/on_sale_filter_pos'),
        );
    ...
    protected function getAllOptions()
    {
        ...
        $options = array_merge($options, $this->getMappedOptions());
    ...
    protected function getMappedOptions()
    {
        ...
        foreach ($mappedOptions as $filterOption) {
            ...
            $tmp['value'] = $filterOption->getCurrentTitle();
            $tmp['attribute_id'] = $filterOption->getAttributeId();
            $tmp['url_alias'] = $filterOption->getUrlAlias();
            $tmp['option_id'] = Amasty_Shopby_Helper_Attributes::MAPPED_PREFIX . $filterOption->getValueId();
            $tmp['disable_seo_url'] = $filterOption->getDisableSeoUrl();
            $optionArray[] = $tmp;
    ...
    {
        if ($this->valueLinkHash === null) {
            $this->valueLinkHash = array();
            foreach (Mage::getModel('amshopby/value_link')->getCollection() as $link) {
                $this->valueLinkHash[$link->getParentId()][] = $link->getOptionId();
            }
        }
        return $this->valueLinkHash;
    }
