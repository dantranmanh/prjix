 ...
 */
class Amasty_Shopby_Model_Catalog_Layer_Filter_Category extends Amasty_Shopby_Model_Catalog_Layer_Filter_Category_Pure
{
    /**
     * Display Types
     */
    ...
    public function addCounts($categories)
    {
        ...
        if (method_exists($collection, '_applySearchFilters')) {
            //trigger applying search filter in catalogsearch fulltext collection on magento 1.9.3.x
            $collection->getSize();
        }
    ...
    protected function getSubcategoriesHeadingData(Mage_Catalog_Model_Category $startCategory)
    {
        ...
        if ($rootId != $startCategory->getId()) {
            ...
            if ($parent->getId() != $rootId && !$this->isExcluded($parent->getId())){
                $parent->setNullProductCount(true);
    ...
    protected function _getProductCount($category)
    {
        if ($this->_getDataHelper()->useSolr()) {
            ...
            return $category->getNullProductCount() ? null : $category->getProductCount();
        }
    }
    ...
    protected function getCategoryApplyData($catId)
    {
        $urlBuilder = $this->urlBuilder;
        $urlBuilder->reset();
        $additionalData = $urlBuilder->getAttributeOptionConfig('cat', $catId);
