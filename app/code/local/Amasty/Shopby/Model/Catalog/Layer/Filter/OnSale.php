...
class Amasty_Shopby_Model_Catalog_Layer_Filter_OnSale extends Mage_Catalog_Model_Layer_Filter_Abstract
{
    ...
    {
        return Mage::helper('amshopby')->__('On Sale Filter');
    }
    ...
    protected function checkAddStatusToSelect(Varien_Db_Select $select)
    {
        if (strpos($select, 'specialPriceTo') === false) {
            $this->setSpecialPriceInSelect($select);
            $this->setCatalogRuleInSelect($select);
        }
    }

    protected function setSpecialPriceInSelect(Varien_Db_Select $select)
    {
        ...
        $select->where('specialPriceFrom.value < NOW()');
        $select->where('specialPriceTo.value > NOW() OR specialPriceTo.value IS NULL');
    }

    protected function setCatalogRuleInSelect(Varien_Db_Select $select)
    {
        /** @var Mage_Core_Model_Resource $resource */
        $resource = Mage::getSingleton('core/resource');
        ...
        $select->joinLeft(
            ...
            AND `catalogRule`.website_id=' .Mage::app()->getStore()->getWebsiteId().'
            AND `catalogRule`.customer_group_id =' .Mage::getModel("customer/customer")->getGroupId(),
            null
        );

        $select->orWhere('catalogRule.product_id IS NOT NULL');
    ...
    protected function getAttributeIds()
    {
        if (is_null($this->attributeIds)) {
            $codes = array('special_from_date', 'special_to_date');
