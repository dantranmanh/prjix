...
class Amasty_Customerattr_Model_Rewrite_Customer_Entity_Attribute
    extends Mage_Customer_Model_Entity_Attribute
{
    public function saveAttributeConfiguration($attributeId, $configuration)
    {
        $this->_getWriteAdapter()->update(
            $this->getTable('customer/eav_attribute'), $configuration,
                ...
                foreach ($option['value'] as $optionId => $values) {
                    ...
                    if (!$intOptionId) {
                        $data = array(
                            ...
                            'sort_order'   => isset($option['order'][$optionId])
                                ? $option['order'][$optionId] : 0,
                        );
                        if (isset($option['parent'])) {
                            $data['parent_option_id']
                                = isset($option['parent'][$optionId])
                                ? $option['parent'][$optionId] : 0;
                        } else if (isset($option['group_id'])) {
                            $data['group_id']
                                = isset($option['group_id'][$optionId])
                                ? $option['group_id'][$optionId] : 0;
                        }
