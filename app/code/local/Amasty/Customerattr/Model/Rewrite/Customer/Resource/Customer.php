...
class Amasty_Customerattr_Model_Rewrite_Customer_Resource_Customer
    extends Mage_Customer_Model_Resource_Customer
{
    public function loadByAttribute(Mage_Customer_Model_Customer $customer,
        $attributeValue, $attribute
    ) {
        $select = $this->_getReadAdapter()->select()
            ->from(
                array('e' => $this->getEntityTable()),
                array($this->getEntityIdField())
            )
            ->joinLeft(
                array('attr' => $this->getEntityTable() . '_'
        ...
        if ($id = $this->_getReadAdapter()->fetchOne(
            $select, array('attribute_id' => $attribute->getId(),
    ...
    public function checkCustomerId($customerId)
    {
        ...
        if ($result) {
            ...
            if (Mage::getStoreConfig(
                ...
                && Amasty_Customerattr_Model_Rewrite_Customer::ACTIVATION_STATUS_ACTIVATED
                !== $customer->getAmIsActivated()
            ) {
                return false;
            }
        }
