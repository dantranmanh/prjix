    ...
    extends Mage_Customer_Model_Customer
{
    ...
    protected function _afterSave()
    {
        if ($this->_fileAttributes) {
            ...
            foreach ($this->_fileAttributes as $id => $attributeCode) {
                ...
                if ($deleteFiles
                && isset($deleteFiles[$attributeCode])
                && ($this->_savedFileNames[$attributeCode] === $deleteFiles[$attributeCode])) {
    ...
    {
        if (Mage::helper('core')->isModuleEnabled('MagePsycho_Customerregfields')) {
            $this->setMpGroupCode(Mage::app()->getRequest()->getPost('mp_group_code'));
        }
        parent::_beforeSave();
        $checkUnique = array();
        $nameGroupAttribute = '';
        ...
        foreach ($collection as $attribute) {
            ...
            && array_key_exists($attribute->getAttributeCode(), $customerAttributes)) {
        ...
        if ($customerAttributes) {
            ...
            foreach ($filesFields as $attributeCode) {
                if (isset($_FILES['amcustomerattr']['error'][$attributeCode])
                    ...
                    $_FILES['amcustomerattr']['name'][$attributeCode]
                        = $attributeValue;
                } elseif (isset($deleteFiles[$attributeCode])
                && ($this->_savedFileNames[$attributeCode] === $deleteFiles[$attributeCode])) { // check if file mark for delete
                    $attributeValue = '';
                } else {
