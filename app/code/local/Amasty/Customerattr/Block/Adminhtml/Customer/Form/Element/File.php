 ...
 */
class Amasty_Customerattr_Block_Adminhtml_Customer_Form_Element_File
    extends Mage_Adminhtml_Block_Customer_Form_Element_File
{
    ...
    protected function _getPreviewHtml()
    {
        ...
        if ($this->getValue() && !is_array($this->getValue())) {
            ...
            $html .= '<span>';
            $html .= '<a href="' . $url . '">' . $this->_drawElementHtml(
                    'img', $image
                ) . '</a> ';
    ...
    protected function _getPreviewUrl()
    {
        ...
        $attributeId = Mage::getModel('eav/entity_attribute')->getIdByCode(
            'customer', $this->getId()
        );
        return Mage::helper('amcustomerattr')->getAttributeFileUrl(
            $customerId, $attributeId, $this->getValue(), true
        );
    }
}
