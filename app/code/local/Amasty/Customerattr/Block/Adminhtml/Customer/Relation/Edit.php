 ...
 */
class Amasty_Customerattr_Block_Adminhtml_Customer_Relation_Edit
    extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_updateButton(
            'save', 'label',
            Mage::helper('amcustomerattr')->__('Save Relation'), 0, 3
        );

        $this->_formScripts[] = " function newAttribute(){ setLocation('"
            . $this->getUrl('adminhtml/amcustomerattrmanage/new') . "');} ";
        $this->_formScripts[] = " function newRelation(){ setLocation('"
            . $this->getUrl('adminhtml/amcustomerattrrelation/new') . "');} ";
        ...
        $this->_formScripts[]
            ...
            function amcustomerattr_load_attr_options()
            {
                ...
                '*/*/options', array('isAjax' => true)
            ) . "?attributeId=' + $('attribute_id').value, {
	                parameters: {
    				},
    ...
    public function getHeaderText()
    {
        if (Mage::registry('entity_relation')->getId()) {
            ...
            return Mage::helper('catalog')->__(
                'Edit Relation "%s"', $this->htmlEscape($frontendLabel)
            );
        } else {
    ...
    {
        return $this->getUrl(
            '*/*/save', array('_current' => true, 'back' => null)
        );
    }
}
