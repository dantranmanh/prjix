 ...
 */
class Amasty_Customerattr_Block_Adminhtml_Customer_Attribute_Edit_Tab_Images
    extends Mage_Adminhtml_Block_Widget
{
    ...
    protected function _doUpload()
    {
         ...
         * Deleting
         */
        $toDelete = Mage::app()->getRequest()->getPost(
            'amcustomerattr_icon_delete'
        );
        if ($toDelete) {
         ...
         * Uploading files
         */
        if (isset($_FILES['amcustomerattr_icon'])
            && isset($_FILES['amcustomerattr_icon']['error'])
        ) {
            foreach (
                $_FILES['amcustomerattr_icon']['error'] as $optionId =>
                $errorCode
            ) {
                if (UPLOAD_ERR_OK == $errorCode) {
                    ...
                    move_uploaded_file(
                        $_FILES['amcustomerattr_icon']['tmp_name'][$optionId],
                        $uploadDir . $optionId . '.jpg'
                    );
                }
            }
        }
    ...
    {
        return Mage::helper('amcustomerattr')->getAttributeImageUrl(
            $option->getId()
        );
    }
