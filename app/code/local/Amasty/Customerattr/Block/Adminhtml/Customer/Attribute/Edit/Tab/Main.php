 ...
 */
class Amasty_Customerattr_Block_Adminhtml_Customer_Attribute_Edit_Tab_Main
    extends Mage_Adminhtml_Block_Widget_Form
{
    protected function _prepareForm()
    {
        ...
        if (!Mage::app()->isSingleStoreMode()) {
            $model->setData(
                'stores', explode(',', $model->getData('store_ids'))
            );
        }
        ...
        $fieldset = $form->addFieldset(
            'base_fieldset',
            array('legend' => Mage::helper('amcustomerattr')->__(
        ...
        $fieldset->addField(
            ...
            'title'    => Mage::helper('amcustomerattr')->__(
                'Attribute Code - for internal use. Must be unique with no spaces'
            ),
            'note'     => Mage::helper('amcustomerattr')->__(
        ...
        $scopes = array(
            Mage_Catalog_Model_Resource_Eav_Attribute::SCOPE_STORE   => Mage::helper(
                'amcustomerattr'
            )->__('Store View'),
            Mage_Catalog_Model_Resource_Eav_Attribute::SCOPE_WEBSITE => Mage::helper(
                'amcustomerattr'
            )->__('Website'),
            Mage_Catalog_Model_Resource_Eav_Attribute::SCOPE_GLOBAL  => Mage::helper(
                'amcustomerattr'
            )->__('Global'),
        );

        if ($model->getAttributeCode() == 'status'
            || $model->getAttributeCode() == 'tax_class_id'
        ) {
        ...
        $ordinaryValidationRules = array(
            ...
            array(
                ...
                'label' => Mage::helper('amcustomerattr')->__(
                    'Letters(a-zA-Z) or Numbers(0-9)'
                )
            ),
        );
        $additionalValidationRules = array();
        $additionalValidationRules = Mage::getModel('amcustomerattr/validation')
            ->getAdditionalValidation();
        $validationRules = array_merge(
            $ordinaryValidationRules, $additionalValidationRules
        );

        $fieldset->addField(
            'frontend_input', 'select', array(
            'name'   => 'frontend_input',
            'label'  => Mage::helper('amcustomerattr')->__(
        ...
        $fieldset->addField(
            'is_unique', 'select', array(
            'name'   => 'is_unique',
            'label'  => Mage::helper('amcustomerattr')->__('Unique Value'),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Unique Value - not shared with other customers'
            ),
            'note'   => Mage::helper('amcustomerattr')->__(
        ...
        $fieldset->addField(
            'file_dimentions', 'text', array(
            'name'  => 'file_dimentions',
            'label' => Mage::helper('amcustomerattr')->__('Image Dimentions'),
            'title' => Mage::helper('amcustomerattr')->__(
                'Image Dimentions - in pixels like: 30/40 (where 30 - width, 40 - height)'
            ),
            'note'  => Mage::helper('amcustomerattr')->__(
                'In pixels like: 30/40 (where 30 - width, 40 - height)'
            ),
        )
        );
        ...
        $fieldset->addField(
            'used_in_order_grid', 'select', array(
            'name'   => 'used_in_order_grid',
            'label'  => Mage::helper('amcustomerattr')->__(
                'Show on the Orders Grid'
            ),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Show on the Orders Grid'
            ),
        ...
        $fieldset->addField(
            'on_order_view', 'select', array(
            'name'   => 'on_order_view',
            'label'  => Mage::helper('amcustomerattr')->__(
                'Show on the Order View page'
            ),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Show on the Order View page - in the Account Information block at the Backend'
            ),
            'note'   => Mage::helper('amcustomerattr')->__(
                'In the Account Information block at the Backend'
            ),
            'values' => $yesno,
        )
        );
        ...
        $fieldset->addField(
            ...
            'label'  => Mage::helper('amcustomerattr')->__(
                'Show on the Account Information page'
            ),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Show on the Account Information page - on the Frontend'
            ),
        ...
        $fieldset->addField(
            'account_filled', 'select', array(
            'name'   => 'account_filled',
            'label'  => Mage::helper('amcustomerattr')->__(
                'Hide if Filled'
            ),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Do not Show if Filled - on the Account Information page on the Frontend'
            ),
            'note'   => Mage::helper('amcustomerattr')->__(
                'On the Account Information page on the Frontend'
            ),
            'values' => $yesno,
        )
        );
        ...
        $fieldset->addField(
            ...
            'label'  => Mage::helper('amcustomerattr')->__(
                'Show on the Billing page'
            ),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Show on the Billing page - during Checkout'
            ),
        ...
        $fieldset->addField(
            'billing_filled', 'select', array(
            'name'   => 'billing_filled',
            'label'  => Mage::helper('amcustomerattr')->__(
                'Hide if Filled'
            ),
            'title'  => Mage::helper('amcustomerattr')->__(
                'Do not Show if Filled - on the Billing page during Checkout'
            ),
            'note'   => Mage::helper('amcustomerattr')->__(
                'On the Billing page during Checkout'
            ),
            'values' => $yesno,
        )
        );
        ...
        $fieldset->addField(
            'sorting_order', 'text', array(
            'name'  => 'sorting_order',
            'label' => Mage::helper('amcustomerattr')->__('Sorting Order'),
            'title' => Mage::helper('amcustomerattr')->__(
                'Sorting Order - the order to display field on Frontend'
            ),
            'note'  => Mage::helper('amcustomerattr')->__(
                'The order to display field on frontend'
            ),
        )
        );
        ...
        if ($model->getId()
            && isset($rewriteAttributeValue[$model->getAttributeCode()])
        ) {
            foreach (
                $rewriteAttributeValue[$model->getAttributeCode()] as $field =>
                $value
            ) {
    ...
    {
        return array(
            'apply' => Mage::getConfig()->getBlockClassName(
                'adminhtml/catalog_product_helper_form_apply'
            )
        );
    }
