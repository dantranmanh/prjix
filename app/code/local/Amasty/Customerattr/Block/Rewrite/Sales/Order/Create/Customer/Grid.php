 ...
 */
class Amasty_Customerattr_Block_Rewrite_Sales_Order_Create_Customer_Grid
    extends Mage_Adminhtml_Block_Sales_Order_Create_Customer_Grid
{
    ...
    public function __construct()
    {
        ...
        if (Mage::getStoreConfig('amcustomerattr/general/select_grid')) {
            $this->_attributeCollection = Mage::getModel('customer/attribute')
                ->getCollection();
            $filters = array(
                           "is_user_defined = 1",
                           "entity_type_id  = ".
                                 Mage::getModel("eav/entity")->setType(
                                     "customer"
                                 )->getTypeId()
            );
            $this->_attributeCollection = Mage::helper('amcustomerattr')
                ->addFilters(
    ...
    protected function _prepareCollection()
    {
        ...
        $collection->joinAttribute(
            'billing_postcode', 'customer_address/postcode', 'default_billing',
            null, 'left'
        )
            ->joinAttribute(
            ...
            ->joinAttribute(
                'billing_regione', 'customer_address/region', 'default_billing',
                null, 'left'
            )
            ->joinAttribute(
                'billing_country_id', 'customer_address/country_id',
                'default_billing', null, 'left'
            )
            ->joinField(
                'store_name', 'core/store', 'name', 'store_id=store_id', null,
                'left'
            );
