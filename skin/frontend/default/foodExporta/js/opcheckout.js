...
Checkout.prototype = {
    ...
    gotoSection: function (section, reloadProgressBlock) {
        ...
        // HungDQ - fix bug document & shippingrate not show when backing to shippping method step - 8/7/2016.
        if(section == 'shipping_method'){
            // if(accordion.currentSection == 'opc-shipping_method'){
                jQuery(".shipping-rate-table").show();
            // }
        }
...
//        //this.accordion.openNextSection(true);
    ...
    back: function(){
        ...
        var stepProgress = this.step2s = [
            {'id':'addressStep', 'action': 'opc-billing'},
            {'id':'addressStep2', 'action': 'opc-shipping'},
            {'id':'deliveryStep' , 'action': 'opc-shipping_method'},
            {'id':'paymentStep' , 'action': 'opc-payment'},
            {'id':'reviewStep' , 'action': 'opc-review'}
        ];

        this.step2s.each(function(step,index){
            if(('opc-' + section) == step.action){
                var index2 = index + 1;
                if(custom_checkout){
                    if(step.id != 'addressStep'){
                        custom_checkout.remove_step(stepProgress[index2].id);
                    }
                }
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    syncWithBilling: function () {
        ...
        if (custom_checkout) {
            custom_checkout.active_step('deliveryStep');
        }
    },
...
ShippingMethod.prototype = {
    ...
    save: function(){
        ...
        if (checkout.loadWaiting!=false) return;
        if (this.validator.validate() && this.validateRate()) {
    ...
    saveSelect: function(vId){
    if (checkout.loadWaiting!=false) return;
    //if (this.validator.validate() && this.validate()) {
    //    checkout.setLoadWaiting('shipping-method');
        jQuery("span#subtotal"+vId).css('opacity','0.25');
        jQuery("img#updateSubtotal-"+vId).show();
        var request = new Ajax.Request(
            this.saveUrl,
            {
                method:'post',
                ...
                onSuccess: function(res){
                    ...
                    for (var key in subtotal) {
                        if (subtotal.hasOwnProperty(vId)) {
                            jQuery("img.wait").hide();
                            //jQuery("span#subtotal"+vId).hide();
                            jQuery("span#subtotal"+vId).html(subtotal[vId]).fadeIn('slow').css('opacity','1');
                        }
                    }
                },
                onFailure: checkout.ajaxFailure.bind(checkout),
    ...
    //Select Logistic Shippingrate
    selectShippingRate: function(shippingRateId, ajaxUrl,price_rate){
        jQuery("#logistic-total").css('opacity','0.25');
        jQuery("#logistic-total-loading").show();
      $('price-update-shipping-rate').innerHTML = price_rate;
        var request = new Ajax.Request(
            ajaxUrl + 'id/' + shippingRateId + '/',
            {
                method:'post',
                onComplete: this.onComplete,
                onSuccess: function(res){
                    var result = (JSON.parse(res.responseText));
                    jQuery("#forex_section").html(result.forex_html);
                    jQuery("#logistic-total").html(result.html).fadeIn('slow').css('opacity','1');
                    jQuery("#logistic-total-loading").hide();
                },
                onFailure: checkout.ajaxFailure.bind(checkout)
            }
        );
    ...
    selectDocumentation: function(documentationId, ajaxUrl, price_document){
        jQuery("#logistic-total").css('opacity','0.25');
        jQuery("#logistic-total-loading").show();
        $('price-update').innerHTML = price_document;
        var request = new Ajax.Request(
            ajaxUrl + 'id/' + documentationId + '/',
            {
                method:'post',
                onComplete: this.onComplete,
                onSuccess: function(res){
    ...
    //Forex Financier - HungDQ
    selectFinancier: function(financierId, ajaxUrl){
        var request = new Ajax.Request(
            ajaxUrl + 'id/' + financierId + '/',
            {
                method:'post',
                onComplete: this.onComplete,
                onSuccess: function(res){
    ...
    nextStep: function(transport){
        if (transport && transport.responseText){
            try{
                ...
                /* active step in progress bar */
                if (custom_checkout) {
                    custom_checkout.active_step('paymentStep');
                }
            }
            catch (e) {
                response = {};
        ...
        if (response.goto_section) {
            ...
            jQuery('body').animate({scrollTop:jQuery("#checkoutSteps").offset().top -50}, '500');
            return;
        }
    ...
    nextStep: function(transport){
        if (transport && transport.responseText){
            try{
                ...
                /* active step in progress bar */
                if (custom_checkout) {
                    custom_checkout.active_step('reviewStep');
                }
            }
            catch (e) {
                response = {};
