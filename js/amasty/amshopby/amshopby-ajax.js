...

{
    ...
    if (!createdElement) {
        ...
        var elements = $$('div.fillter-products');
...
function amshopby_ajax_request(url){
	$extrat=jQuery(".tab-title-second.active")
	var exid=$extrat.attr('id');
	var param= exid.replace("am_","amf=");				
	url.replace("amf=all",param); 
	url.replace("amf=new",param);
	url.replace("amf=sale",param);
	url.replace("amf=instock",param);
    /*
	...
	jQuery("#emptycollection").removeClass("emptycollection");
    ...
    var request = new Ajax.Request(url,{
            ...
            onSuccess: function(response){
				...
				jQuery(".arrow").each(function() {
				 ...
				 /**if(jQuery(".emptycollection") != undefined) jQuery(".toolbar").hide(); **/
    ...
    for (var id in blocks){
        ...
        if (html){
            ...
            if (block){
                ...
                var targetElement = amshopby_get_first_descendant(tmp);
				if(!(id = "amshopby-filters-left" && jQuery(".emptycollection") != undefined)) 
                block.parentNode.replaceChild(targetElement, block);
...
function copy_toolbar(){	/** This must be work in category layer page and search page **/	if(jQuery('.tb-copy') != undefined && jQuery('.tb-copy').length){		jQuery("#amshopby-page-container .toolbar").each(function() {			/**if(jQuery('.tb-copy .toolbar').length > 0 )  */			jQuery('.tb-copy .toolbar').remove();			jQuery( this ).detach().appendTo('.tb-copy');			jQuery('.toolbar').hide();			jQuery('.tb-copy .toolbar').show();		  });		}	if(jQuery('.cat-image') != undefined && jQuery('.cat-image').length){		jQuery("#amshopby-page-container .category-top-image").each(function() {						jQuery('.cat-image .category-top-image').remove();			jQuery( this ).detach().appendTo('.cat-image');			jQuery('.category-top-image').hide();			jQuery('.cat-image .category-top-image').show();		  });		}	}
